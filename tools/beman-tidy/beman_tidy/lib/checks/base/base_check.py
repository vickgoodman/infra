...
class BaseCheck(ABC):
    ...
    def __init__(self, repo_info, beman_standard_check_config, name=None):
        ...
        self.config = beman_standard_check_config[self.name] if not "INTERNAL." in self.name else None

        # set type - e.g. "REQUIREMENT" or "RECOMMENDATION"
        self.type = beman_standard_check_config[self.name]["type"] if not "INTERNAL." in self.name else "REQUIREMENT"
        ...
        self.full_text_body = beman_standard_check_config[self.name]["full_text_body"] if not "INTERNAL." in self.name else ""
